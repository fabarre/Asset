...
* **Voci CAPEX/OPEX personalizzate per impianto**: tabella `plant_custom_costs` per dichiarare costi extra (€ totale o €/kWp, con **aliquota IVA per riga**, colonna `vat_rate`), integrati in CAPEX/OPEX/ammortamenti/EBITDA e nel cash flow; salvataggio automatico nel database alla conferma della riga. Nel modello budget della card Cash Flow: contatori **Totale/Allocato/Residuo** (netti) per CAPEX e OPEX Anno 1 con categorie (EPC FV/BESS, connessione, sviluppo, SPV, terreno / O&M, assicurazione, IMU, ecc.); ogni riga esborso/evento mostra **Netto / +IVA / Lordo** con IVA calcolata automaticamente dall'aliquota della voce, e i pannelli espongono **IVA allocata** e **Lordo allocato**; il residuo CAPEX va al COD, il residuo OPEX si spalma /12.
* **IVA nel solo Cash Flow**: IVA di cassa come pass-through (a debito sui ricavi incassati, a credito su CAPEX+OPEX con **aliquote per categoria di spesa** e per voce personalizzata, liquidazione mensile/trimestrale con credito portato a nuovo). Incide solo sul timing della cassa, **non su P&L né IRR**. Configurabile in FINANZA: abilita, aliquota sui ricavi, % ricavi imponibili, frequenza liquidazione e 12 aliquote per voce (EPC FV/BESS, connessione, sviluppo, SPV, terreno / O&M FV/BESS, assicurazione, IMU, sicurezza, asset management — default italiani: terreno/IMU/assicurazione 0%, resto 22%).
...
* **Test automatici**: `npm test` → `tests/test_worker.mjs` (**143 assertion** sul motore: fisco, SoC, grace, sculpting, MSD, Monte Carlo seedato, BRP per anno, CER, quadrature mensili, cash flow 72 mesi, IVA di cassa per categoria, edge case); `npm run test:e2e` → harness Playwright/Chromium for Testing su app live (KPI, tab, gate anonimi, notifiche); `npm run test:xss` → 13 check di escaping con payload reali.
