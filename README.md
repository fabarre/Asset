...
* **Cash flow mensile anni 1-5**: schedule a 60 mesi (ricavi per stream con stagionalità reale, OPEX/imposte/servizio debito, cassa apertura/chiusura) con quadratura esatta sui totali annui; card dedicata in Dashboard, foglio Excel "CASH FLOW MENSILE" e pagina nel report Rendiconto Finanziario.
* **Qualità Dati & Copertura**: card in Dashboard con score 0-100 e check automatici (copertura PUN 8760h per zona, NaN/negativi/zeri, integrità e yield curve impianti, carichi stabilimenti, quadratura energetica Y1, sanity leva) + export CSV del report.
...
* **Test automatici**: `npm test` → `tests/test_worker.mjs` (90 assertion sul motore: fisco, SoC, grace, sculpting, MSD, Monte Carlo seedato, BRP per anno, CER, quadrature mensili, edge case); `npm run test:e2e` → harness Playwright/Chromium for Testing su app live (KPI, tab, gate anonimi, notifiche); `npm run test:xss` → 13 check di escaping con payload reali.
